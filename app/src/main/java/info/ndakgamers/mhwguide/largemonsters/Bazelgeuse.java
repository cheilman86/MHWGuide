...
public class Bazelgeuse extends Fragment {
    ...
    private void loadMonster() {
        ...
        try {
            monsterCursor = myDbHelper.getReadableDatabase().rawQuery("select * from large_monsters where monsterName like \"Bazel%\"", null);
    ...
    private void loadMonsterItems() {
        ...
        try {
            myDbHelper.openDataBase();
            monsterItemCursor = myDbHelper.getReadableDatabase().rawQuery("select * from materials where (materialname like 'Bazel%') or (materialname like '%hardbone')", null);
