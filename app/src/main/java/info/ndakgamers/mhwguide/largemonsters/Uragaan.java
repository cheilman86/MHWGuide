...
public class Uragaan extends Fragment {
    ...
    private void loadMonster() {
        ...
        //Creating our Cursor
        Cursor monsterCursor = myDbHelper.getReadableDatabase().rawQuery("select * from large_monsters where monsterName like \"Uraga%\"", null);
    ...
    private void loadMonsterItems(){
        myDbHelper.openDataBase();
        Cursor monsterItemCursor = myDbHelper.getReadableDatabase().rawQuery("select * from materials where (materialname like 'Uraga%') or (materialname like 'flame sac') or (materialname like 'inferno sac')",null);
