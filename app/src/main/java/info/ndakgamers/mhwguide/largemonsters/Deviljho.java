...
public class Deviljho extends Fragment {
    ...
    private void loadMonster() {
        ...
        //Creating our Cursor
        Cursor monsterCursor = myDbHelper.getReadableDatabase().rawQuery("select * from large_monsters where monsterName like \"Devil%\"", null);
    ...
    private void loadMonsterItems(){
        myDbHelper.openDataBase();
        Cursor monsterItemCursor = myDbHelper.getReadableDatabase().rawQuery("select * from materials where (materialname like 'Deviljho%') " +
                "or (materialname like '%hardbone')",null);
