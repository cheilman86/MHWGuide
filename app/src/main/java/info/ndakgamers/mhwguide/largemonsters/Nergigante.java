...
public class Nergigante extends Fragment {
    ...
    private void loadMonster() {
        ...
        //Creating our Cursor
        Cursor monsterCursor = myDbHelper.getReadableDatabase().rawQuery("select * from large_monsters where monsterName like \"Nergig%\"", null);
    ...
    private void loadMonsterItems(){
        myDbHelper.openDataBase();
        Cursor monsterItemCursor = myDbHelper.getReadableDatabase().rawQuery("select * from materials where (materialname like 'Nergig%') " +
                "or (materialname like '%dragonscale') " +
                "or (materialname like 'elder%')",null);
